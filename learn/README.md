...
- [Helm](#helm)
  ...
  - [Helm: Deep Dive into Helm Charts](#helm-deep-dive-into-helm-charts)
...
- [Kustomize](#kustomize)
  - [Kustomize: Why](#kustomize-why)
  - [Kustomize: Concepts](#kustomize-concepts)
  - [Kustomize: `kustomization.yaml`](#kustomize-kustomizationyaml)
  - [Kustomize: Building, Applying and Deleting](#kustomize-building-applying-and-deleting)
  - [Kustomize: Managing Directories](#kustomize-managing-directories)
...
Helm is often called a package manager for Kubernetes. That is because, the deployment of an application usually involves not just the pod and service but multiple Kubernetes objects like Secrets, ConfigMaps, PersistentVolumes, PersistentVolumeClaims and so on. All of these together make the application work. Helm makes installing the application simple by working like a package manager for the application.

## Helm: Concepts

1. **Charts**: Charts are a collection of files. They contain all the instructions that helm needs to know to be able to create the collection of objects that one needs in a Kubernetes Cluster. By using charts and adding the objects according to the specific instructions in the charts, Helm installs applications into a Kubernetes Cluster.
2. **Releases**: When a Chart is applied to a Cluster, a Release is created. A release is a single installation of an application using a Helm chart.
3. **Revisions**: Within each Release you can have multiple revisions. Each revision is like a snapshot of the application. Every time a change is made to the application, such as an update of the image or change in replicas or configuration objects, a new revision is created.
4. **Metadata**: Finally, to keep track of what it did to our cluster such as the releases that it installed, the charts used, revision state, and so on, Helm will need a place to save this data. This is known as metadata (data about data). It wouldn't be useful to save this on a local computer. If another person were to work with our releases, they would need a copy of this data. Instead, Helm does the smart thing and saves this metadata directly in our Kubernetes cluster as Kubernetes Secrets. This way the data survives and as long as the Kubernetes cluster survives, everyone from the team can access it. Thus, this way, Helm will always know about everything done in this cluster via the metadata.

Directory structure can look like this:
...
> **Note**: Install the Kubernetes plugin for VS Code for a clean linting experience.

---

### Charts

Helm knows how to do its job with the help of Charts. Charts are like an instruction manual for it. By reading and interpreting its contents, Helm knows what it has to do to fulfil an user's request. From the human operator's perspective, charts are a bunch of yaml files.

Each specific file named in a specific way has a well-defined purpose.

- `values.yaml`: Templating is done via a `values.yaml` file which replaces the template variables.
- `chart.yaml`: Contains information about the chart itself such as:
  - `apiVersion`: v2 # (v1 or v2),
...
The `values.yaml` file is where the configurable values are stored. Most of the time, this is the only file that you will need to modify to customize the deployment of the application for your needs.

Templating can be complex and we will get more into it later.
...
type: application # application (default) or library (type of chart that provides utilities that help in building charts)
dependencies:
...
- `templates`: (Directory) Templates directory containing the manifest files with templating.
- `values.yaml`: (File) Provide configurable values to the templates.
- `chart.yaml`: (File) Information about the chart.
- `LICENSE`: (File) License of the chart.
- `README.md`: (File) Human readable instructions.
- `charts`: (Directory): Optional directory. Other charts this chart is dependent upon.
...
We could ask ourselves, why the need for an additional item? Why can't we just say, we installed a chart to Kubernetes?
...
The simple reason, is that we can have multiple releases based on the same chart.
...
```bash
# Search artifacthub.io for wordpress charts
...
```bash
# Search repo (Bitnami Helm Repository) for wordpress charts
helm search repo wordpress
```

```bash
# List existing chart repositories
helm repo list
```
...
Each time we pull a chart and install it, a release is created. A release represents a package or collection of Kubernetes objects together representing an application's instance deployment.
...
# Learn more details about a particular release
helm history [release-name]
helm history nginx-release

# Return a release to a previous state. It creates a new revision, incrementing the revision number, but goes back to the older revision's configuration.
helm rollback [release-name] [revision-number]
helm rollback nginx-release 1
```

> **Note**: Rollbacks do not cover file or directory data that maybe created by the applications. Instead Helm backs up and restores the declarations or manifest files of the Kubernetes objects. So for things that use PersistentVolumes or other forms of persistent data, or something external, like a database storing on a NFS via CSI plugin, a rollback won't restore that data too, i.e. the current data will be retained (neither backed up nor restored).
>
> There are options available to take consistent backups of databases before upgrading charts or even to rollback and restore databases but they are done using Chart Hooks.

---

## Helm: Deep Dive into Helm Charts

### Writing Charts

Besides installing various Kubernetes objects to get our app up and running, charts can do some extra stuff too.

For example, we can write the chart in such a way that whenever we do a Helm upgrade, a database can be automatically backed up before that happens (chart hook to initiate backup). Now we have a way to restore the database, in case something goes wrong.

Let's first start by learning how to create a helm chart.
...
**Template values provided by default**:
...
Built in objects like the `Release`, `Chart` and `Capabilities` have a PascalCase.
Objects defined in `values.yaml` have a camelCase.
...
1. **`helm lint [CHART-PATH]`**: Linting helps us verify that the chart and YAML format is correct.
2. **`helm template [RELEASE-NAME] [CHART-PATH]`**: Verify that the templating part is working as expected, i.e. the templating is generating the correct values as intended.
3. **`helm template [RELEASE-NAME] [CHART-PATH] --debug`**: Debugs formatting errors.
4. **`helm install [RELEASE] [PATH] --dry-run`**: Dry run to ensure that the chart works well with Kubernetes itself. In cases, where the YAML format is correct, only Kubernetes can tell us if a Manifest format is wrong and it failed to create resources, this comes handy.
...
Using Pipes or Pipelines is another common way of writing functions:

In Linux we can do a:

```bash
# Translate Name to NAME
echo "Name" | tr a-z A-Z
```

Similarly in Helm templates we can do:
...
The current scope of a helm chart is the root scope by default. Hence to access a nested object, you have to traverse all the way from the root scope for every value. We can set a scope using the `with` block to avoid repetition.
...
To access root scope within a with block (using `$`):
...
For a `values.yaml` file that looks like:
...
and we want a configmap that looks like this:
...
We can create a configMap that looks like:
...
Repetitive lines can be moved to what is called a partial or a named template. We can move repetitive lines like repeating labels to a file called `_helpers.tpl`.
The underscore in the file name tells helm not to consider this file as a usual template file.
...
Can be imported into a chart using a `template` statement, like so:
...
metadata:
  ...
  labels:
    {{- template "labels" . }} # the dot allows the scope to be accessed
spec:
  ports:
    - port: 80
      targetPort: http
...
The name `labels` is a named template.

> **Note**: The indentation for named templates has to match the destination chart. To fix unavoidable indentations you can use the `indent` function. However it wouldn't work straight out of the box by piping like `{{- template "labels" . | indent 2 }}`. Instead we need to use the `include` function. The `include` function does the same job as the template function i.e. import a named template, but also pipe it to another function.

For example, in a Deployment, the indentation will vary:
...
Earlier we discussed that, besides just installing Kubernetes objects to get our app up and running, Helm charts can do some other stuff too.
For example:

- **Database Backup before Upgrade**: We can write our charts in such a way that whenever we do a `helm upgrade` a database can be automatically backed up before the upgrade, so we have a way to restore the database from backup if anything goes wrong.
- **Email alert before Upgrade**: Put up a site-wide alert before an upgrade commences and remove it after the upgrade.
...
We need to add an annotation with the key `"helm.sh/hook"` to let helm know that this is not a regular helm template but a chart hook.
...
But what about the order of multiple hooks of the same annotation value. Let's say we have three pre-upgrade hooks, but they have to be sent in a certain order.

That is done with the annotation `helm.sh/hook-weight: "<number>"`. Note, that you have to set the value as a string. During chart installation phase, helm sorts this in ascending order.

The job resource stays on created as a resource on the cluster, unless we also add another annotation with a supported value:, `helm.sh/hook-delete-policy: <supported-value>`.

For instance: `helm.sh/hook-delete-policy: hook-succeeded` deletes the Job resource after the hook is successfully executed. `hook-failed` deletes the resource even if the hook fails execution. `before-hook-creation` deletes the last resource that ran (first time nothing happens).
...
The idea of Kustomize is to have a single deployment of an application be done differently on different environments. Say we want to have a Nginx deployment, however on our local machine (development), we want to deploy just 1 x replica, while it maybe different on staging and production.

We could have a solution where we have three directories representing our environments - `development`, `staging` and `production`. The deployment file copied to all three directories, with a change in the `.spec.replicas`. However, while this works, this is not the most ideal solution. Suppose we make some changes, or add a `service.yaml` or some other resource, now the onus is on us to copy that correctly to the other directories as well and if any changes are needed, make changes. This is clearly not a scalable solution, as it is only a matter of time where there is a human error during copying.

This is the reason Kustomize was created, to avoid this duplication, and reuse our Kubernetes configs and only modify what needs to be changed on a per-environment basis.

---

## Kustomize: Concepts

Kustomize is a tool that comes built in with kubectl so no other package installations are necessary. It is not a templating language like Helm, very readable, and doesn't require much additional learning. Every artifact that Kustomize uses is just plain YAML, so it can be processed and validated simply as YAML.

The basic concepts revolve around `base` configs and `overlays` for environments.

1. **`base`**: Any config / resources that are shared across all environments and/or default values (can be overwritten).
2. **`overlays`**: The overlays allow us to customize the behaviour on a per-environment basis.
...
Kustomize takes the base configs and joins them with the overlays and creates the final Kubernetes manifests.

---

## Kustomize: `kustomization.yaml`

Kustomize looks for a specific file, the `kustomization.yaml`.

The anatomy looks like this:

```yaml
# apiVersion and kind are optional but recommended to avoid breaking changes
...
# All customizations that are to be made
commonLabels:
  company: Zenius
```
...
## Kustomize: Building, Applying and Deleting

The `kustomize build [PATH]` command combines all the manifests and applies the defined transformations and shows the output. It does not apply resources to the Kubernetes cluster.

> **Note**: When using the `kustomize` that comes with `kubectl`, `kubectl kustomize [PATH]` works the same way as `kustomize build [PATH]`.

**To apply resources to the cluster**:
...
**To delete resources from the cluster**:

```bash
# Build and Delete
kustomize build [PATH] | kubectl delete -f -
kubectl kustomize [PATH] | kubectl delete -f -
...
When the Kubernetes manifest files in a directory become too many, we may want to create subdirectories to group them.
The problem is when using the Standard `kubectl apply -f` command, now the `kubectl apply -f [DIRECTORY]` command has to be applied multiple times per subdirectory.

Kustomize solves this by allowing us to define the resources in the `kustomization.yaml` file.
...
Now that Kustomize is aware of the various resources in different subdirectories, we can run this on the root:
...
However, although this works, this is not an ideal solution, as the `kustomization.yaml` at the root, gets cluttered fairly soon.
What we can do is, add a `kustomization.yaml` for each subdirectory, where we specify the resources within that particular directory.

So for `db/kustomization.yaml`:
...
As for the root `kustomization.yaml`, we can do this:
